...
int smb347_apsd_process(int apsd, int *wall_charger)
{
	...
	if (smb347_i2c_read(SMB347_STATUS_REG_D, &apsd, 1)) {
		SMB347_LOG("in %s, line %d, failed to read status register D\n", __func__, __LINE__);
		goto done;
	}
	/* for debugging */
	SMB347_LOG("REG-3e = %02x\n", apsd);

	if (smb347_apsd_result(apsd) != -1) {
		SMB347_LOG("%s detected\n", smb347_apsd_result_string(apsd));

		if (smb347_i2c_read(0x06, &tmp, 1)) {
			SMB347_LOG("in %s, line %d, smb347_i2c_read failure\n");
			goto done;
		}
		SMB347_LOG("reg 06 = %02x\n", tmp);

		if (smb347_apsd_result(apsd) == SMB347_APSD_RESULT_DCP &&
				wall_charger)
			*wall_charger = 1;
		else if (smb347_apsd_result(apsd) == SMB347_APSD_RESULT_SDP) {
			/* switching to USB5 mode for SDP */
		...
		} else if (smb347_apsd_result(apsd) == SMB347_APSD_RESULT_OTHER) {
			/*
			 * 3rd party chargers we need to manually enable HC mode
			 */
			if (smb347_i2c_read(SMB347_COMMAND_REG_B, &mode, 1))
				goto done;
...
#if defined(CONFIG_MACH_BOWSER_SUBTYPE_JEM) || defined (CONFIG_MACH_BOWSER) || defined(CONFIG_MACH_BOWSER_SUBTYPE_SOHO)
static int smb347_set_current_setting(void)
{
	int ret = -1;
	u8 value = 0xff;

	if ((ret = smb347_i2c_read(0x0, &value, 1))) {
		SMB347_LOG("%s: Unable to read SMB347_CHARGE_CURRENT: %d\n", __func__, ret);
		goto done;
	}
	...
	if ((ret = smb347_i2c_write(0x0, &value, 1))) {
		SMB347_LOG("%s: Unable to write SMB347_CHARGE_CURRENT: %d\n", __func__, ret);
	}
...
static int smb347_set_current_compensation(void)
{
	int ret = -1;
	u8 value = 0xff;

	if ((ret = smb347_i2c_read(0x0A, &value, 1))) {
		SMB347_LOG("%s: Unable to read SUMMIT_SMB347_OTG_THERM_CONTROL: %d\n", __func__, ret);
		goto done;
	}
	...
	if ((ret = smb347_i2c_write(0x0A, &value, 1))) {
		SMB347_LOG("%s: Unable to write SUMMIT_SMB347_OTG_THERM_CONTROL: %d\n", __func__, ret);
	}
...
static int smb347_set_temperature_threshold(void)
{
	int ret = -1;
	u8 value = 0xff;
	...
	if ((ret = smb347_i2c_write(0x0B, &value, 1))) {
		SMB347_LOG("%s: Unable to write SUMMIT_SMB347_CELL_TEMP: %d\n", __func__, ret);
	}
...
static int smb347_switch_mode(int mode)
{
	int ret = 0;
	u8 value = 0xff;

	if ((ret = smb347_i2c_read(SMB347_COMMAND_REG_B, &value, 1))) {
		SMB347_LOG("%s: Unable to read SMB347_COMMAND_REG_B: %d\n",__func__, ret);
		goto done;
	} else {
		switch (mode) {
		case SMB347_USB1_MODE:
			SMB347_LOG("Switching to USB1 mode\n");
			value &= ~0x01;
			value &= ~0x02;
			break;
		case SMB347_USB5_MODE:
			SMB347_LOG("Switching to USB5 mode\n");
			value &= ~0x01;
			value |= 0x02;
			break;
		case SMB347_HC_MODE:
			SMB347_LOG("Switching to HC mode\n");
			value |= 0x01;
			break;
		default:
			SMB347_LOG("Unknown USB mode: %d\n", mode);
			return -1;
		}

		if ((ret = smb347_i2c_write(SMB347_COMMAND_REG_B, &value, 1))) {
			SMB347_LOG("%s: Unable to write SMB347_COMMAND_REG_B: %d\n", __func__, ret);
			goto done;
		}
		ret = 0;
	}
...
int smb347_init_no_redo()
{
	int status = -1;
	u8 val = 0, vusb = 0, tmp = 0, apsd_enable = 0;
	u32 reg = 0;
	int i2c_addr;
	...
	/* enable apsd if it is disabled */
	if (smb347_i2c_read(0x04, &apsd_enable, 1))
		goto done;
	if (!(apsd_enable & (0x1 << 2))) {
		apsd_enable |= (0x1 << 2);
		if (smb347_config_enable(1))
			goto done;
		if (smb347_i2c_write(0x04, &apsd_enable, 1))
			goto done;
		if (smb347_config_enable(0))
			goto done;
	}
	...
	SMB347_LOG("in %s, Detecting USB source...\n", __func__);
	...
	/* use register control for USB5/1/HC mode */
	if (smb347_i2c_read(0x06, &val, 1))
		goto done;
	val &= ~(0x3 << 5);
	val |= (0x1 << 5);
	val &= ~(0x1 << 4);
	/* set taper current to 150mA */
	if (smb347_i2c_read(0x00, &tmp, 1))
		goto done;
	tmp &= ~(1 << 2);
	tmp |= 0x03;

	SMB347_LOG("in %s, REG-06 = %02x, REG-04 = %02x\n", __func__, val, apsd_enable);
	...
	smb347_set_current_setting();

	/* Set Charge Curretn Comesation to 900mA */
	smb347_set_current_compensation();

	/* Limit Hard/Soft temperature to 60/0(Hard), 45/15(Soft) */
	smb347_set_temperature_threshold();
#endif
	status = 0;
done:
	/* Enable USB PHY */
...
int smb347_init(int *wall_charger)
{
	...
	if (!(vusb & (1 << 2))) {
		...
		SMB347_LOG("Nothing on USB\n");
		i2c_addr = smb347_i2c_probe();
		smb347_switch_mode(SMB347_USB1_MODE);
		status = 0;
		goto done;
	}
	...
	if (smb347_apsd_process(apsd, wall_charger))
		goto done;
