...
/* Set the Lab126 Product ID */
#if defined (CONFIG_MACH_BOWSER_SUBTYPE_TATE)
#define DEVICE_PRODUCT_ID 0x0007
#elif defined (CONFIG_MACH_BOWSER_SUBTYPE_JEM) || defined (CONFIG_MACH_BOWSER_SUBTYPE_SOHO)
#define DEVICE_PRODUCT_ID 0x0008
#elif defined (CONFIG_MACH_BOWSER_SUBTYPE_RADLEY)
#define DEVICE_PRODUCT_ID 0x0009
#else
...
#ifdef CONFIG_FASTBOOT_ENHANCE_SPEED
static int fastboot_dma_to_dram (unsigned int buffer_size)
{
	if (download_size)
	{
		...
		if (buffer_size)
		{
			...
			if (fifo_size == buffer_size)
			{
				...
				*peri_rxcsr |= MUSB_RXCSR_DMAENAB;
				if (read_bulk_fifo_dma(fastboot_interface->transfer_buffer + download_bytes, fifo_size))
				{
					fastboot_rx_error();
				}
				*peri_rxcsr &= ~(MUSB_RXCSR_DMAENAB | MUSB_RXCSR_DMAMODE);
			}
			else
			{
				for (i = download_bytes; i < buffer_size+download_bytes; i++)
				{
					*(fastboot_interface->transfer_buffer + i) = read_bulk_fifo_8();
				}
			}
			*peri_rxcsr &= ~MUSB_RXCSR_RXPKTRDY;
			...
			if (download_bytes &&0 == (download_bytes %(16*10 * fastboot_interface->nand_block_size)))
			{
				if (0)
					printf("X");
				else
					printf(".");
				if (0 == (download_bytes %(80*10 * 16 *fastboot_interface->nand_block_size)))
					printf("\n");
			}
		}
		else
	...
	else
	{
		if (intrusb & OMAP34XX_USB_INTRUSB_SOF)
		{
...
#ifdef CONFIG_FASTBOOT_ENHANCE_SPEED
			{
				...
				{
					while(1){
						if(inw(OMAP34XX_USB_INTRRX))
						{
							ret = fastboot_rx();
							fastboot_resume();
						}
						if(fastboot_magic == 0 || ret)
						{
							fastboot_magic = 0;
							break;
						}
...
static inline void gpio_wk_init(void)
{
	/*
	 * Addition registers need to be configured due to GPIO0
	 * being mux'ed with SIM
	 */
	__raw_writel(__raw_readl(CONTROL_PBIAS_LITE) & (~0x90100000),
					CONTROL_PBIAS_LITE);
	__raw_writel(__raw_readl(CONTROL_GPIOWK) & (~0xB0000000),
					CONTROL_GPIOWK);
	__raw_writel(__raw_readl(CONTROL_GPIOWK) | 0x10000000,
					CONTROL_GPIOWK);
	__raw_writel(0x15700000, CONTROL_PBIAS_LITE);
}

/* return 1 to enter fastboot mode */
...
#define KBD_FULLCODE31_0	(0x4a31c044)
	...
	/* USB_OTG_ID (gpio_104) => Enter Fastboot */
	if ((__raw_readl(OMAP44XX_GPIO4_BASE + DATA_IN_OFFSET) & (1 << 8))) {
		printf("\nBowser: Special cable detected: entering fastboot....\n");
		return 1;
	}


    /* The following code is disabled.  This functionality is performed in idme code */

#if 0 
	/* Volume_down key (gpio_50) => Enter Fastboot */
	if (!(__raw_readl(OMAP44XX_GPIO2_BASE + DATA_IN_OFFSET) & (1 << 18))) {
		printf("\nBowser: Volume_down key pressed: entering fastboot....\n");
		return 1;
	}

	/* Volume_up key (gpio_1) => Enter Recovery */
	if (!(__raw_readl(OMAP44XX_GPIO1_BASE + DATA_IN_OFFSET) & (1 << 1))) {
		printf("\nBowser: Volume_up key pressed: entering recovery....\n");
		goto start_recovery;
	}
#endif
...
start_recovery:
#ifdef CFG_BOOTMSG
			bootmsg_write_command(BOOTMSG_COMMAND_RECOVERY);
#else
...
extern u8 *idme_get_board_serial(void);
extern int idme_get_board_type_rev_string(char *bt);
static char board_type[32];
...
#elif defined(CONFIG_4430SDP) && defined(CONFIG_MACH_BOWSER_SUBTYPE_JEM)
        printf("board_type = %s\n", board_type);
        device_strings[DEVICE_STRING_PRODUCT_INDEX] = board_type;
#elif defined(CONFIG_4430SDP) && defined(CONFIG_MACH_BOWSER_SUBTYPE_TATE)
        printf("board_type = %s\n", board_type);
        device_strings[DEVICE_STRING_PRODUCT_INDEX] = board_type;
#elif defined(CONFIG_4430SDP) && defined(CONFIG_MACH_OTTER2)
        printf("board_type = %s\n", board_type);
#elif defined(CONFIG_4430SDP) && defined(CONFIG_MACH_BOWSER_SUBTYPE_SOHO)
        printf("board_type = %s\n", board_type);
        device_strings[DEVICE_STRING_PRODUCT_INDEX] = board_type;
...
#if defined(CONFIG_ENABLE_IDME)
	static char device_serial[IDME_MAX_BOARD_SERIAL_LEN + 1];
	char *p;

	p = idme_get_board_serial();
	if (p) {
		strncpy(device_serial, p, sizeof(device_serial) - 1);
		device_strings[DEVICE_STRING_SERIAL_NUMBER_INDEX] = device_serial;
	} else {
		device_strings[DEVICE_STRING_SERIAL_NUMBER_INDEX] = "00123";
	}

#elif defined(CONFIG_4430SDP) || defined(CONFIG_4430PANDA)
...
#ifdef CONFIG_MACH_OTTER2
	/* PHY interface is 8-bit, UTMI+ level 3*/
	*otg_interfsel &= 0;
	/* Enable functional PHY clock (OTG_60M_FCLK)*/
