...
int show_bootmsg(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	if (argc == 2) {
		if (strncmp(argv[1], "normal", 6) ==0) { 
			bootmsg_write_command(BOOTMSG_COMMAND_NORMAL);
...
U_BOOT_CMD(
	bootmsg, 2, 1, show_bootmsg,
	"bootmsg - print bootmsg\n",
	"bootmsg [normal/recovery] \n"
);
