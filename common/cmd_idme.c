...
static int idme_generate_default_vars(const unsigned char *pbuf)
{
        ...
        memcpy(pidme_data+nvram_info[0].offset, 
	       IDME_VERSION_STR, strlen(IDME_VERSION_STR));
#if defined (CONFIG_MACH_BOWSER_SUBTYPE_SOHO)
        memcpy(pidme_data+nvram_info[1].offset, 
	       "8080600000000010", 16); /* boardid default SOHO EVT2 */
#endif 
#if defined (CONFIG_MACH_BOWSER_SUBTYPE_JEM)
        memcpy(pidme_data+nvram_info[1].offset, 
	       "8010400000000010", 16); /* boardid default jem PVT */
#endif 

        for (i = 2; i < CONFIG_NUM_NV_VARS; i++){
                if (strncmp(nvram_info[i].name, "bootmode", strlen("bootmode")) == 0){
                        memcpy(pidme_data+nvram_info[i].offset, "1", 1);    /* set initial bootmode to diag kernel */
...
int idme_update_vars_ex(void)
{
        ...
        if (strncmp(boot_buffer, 
		    IDME_MAGIC_NUMBER, 
		    strlen(IDME_MAGIC_NUMBER))){
        ...
        if (memcmp(pidme_data+nvram_sig_info.offset, 
                ...
                for(i = 0; i < CONFIG_NUM_NV_VARS; i++){
                        if((*pbitmap & (1 << i)) != 0){
                                memcpy(pidme_boot_data + nvram_info[i].offset, 
...
#define VOL_UP_KEY 1
#define VOL_DN_KEY 2
/* Get volume key status.
 * Returns:
 *   VOL_UP_KEY: volume up pressed
 *   VOL_DN_KEY: volume down pressed
 */
int idme_get_volume_key(void)
{
...
#else
	/* for production build, enter recovery if volume down is pressed
	 * or directly read the bootmode from emmc
	 */
	if (idme_get_volume_key() == VOL_DN_KEY)
		bootmode[0] = '3';
	else
        ...
        switch(bootmode[0]){
        case '1':
                printf("Select Normal boot image with bootmode=1\n");
                break;
        case '2':
                printf("Select Normal Boot image with bootmode=2\n");
                break;
        case '3':
                ...
                if (!strncmp(pBd, "000", 3)){
				  ...
				  if (!strncmp(&pBd[3], "01", 2)){
					...
					goto unknown_board;
                        ...
                        strncpy(bt, "Radley-Eng-", strlen("Radley-Eng-"));
                        bt += strlen("Radley-Eng-");
                }else if (!strncmp(pBd, "80801", 5)){
                        strncpy(bt, "Soho-Proto-Dummy-", strlen("Soho-Proto-Dummy-"));
                        bt += strlen("Soho-Proto-Dummy-");
                }else if (!strncmp(pBd, "80802", 5)){
                        strncpy(bt, "Soho-PreEVT1-Dummy-", strlen("Soho-PreEVT1-Dummy-"));
                        bt += strlen("Soho-PreEVT1-Dummy-");
                }else if (!strncmp(pBd, "80803", 5)){
                        strncpy(bt, "Soho-EVT1-Eng-", strlen("Soho-EVT1-Eng-"));
                        bt += strlen("Soho-EVT1-Eng-");
                }else if (!strncmp(pBd, "80804", 5)){
                        strncpy(bt, "Soho-PreEVT2-Eng-", strlen("Soho-PreEVT2-Eng-"));
                        bt += strlen("Soho-PreEVT2-Eng-");
                }else if (!strncmp(pBd, "80805", 5)){
                        strncpy(bt, "Soho-EVT2-Eng-", strlen("Soho-EVT2-Eng-"));
                        bt += strlen("Soho-EVT2-Eng-");
                }else if (!strncmp(pBd, "80806", 5)){
