...
#define MMC_CMD56	(INDEX(56) | RSP_TYPE_R1   | DP_DATA 	| DDIR_READ)
...
#define MMC_R1_OUT_OF_RANGE         (1 << 31)       /* er, c */
#define MMC_R1_ADDRESS_ERROR        (1 << 30)       /* erx, c */
#define MMC_R1_BLOCK_LEN_ERROR      (1 << 29)       /* er, c */
#define MMC_R1_ERASE_SEQ_ERROR      (1 << 28)       /* er, c */
#define MMC_R1_ERASE_PARAM          (1 << 27)       /* ex, c */
#define MMC_R1_WP_VIOLATION         (1 << 26)       /* erx, c */
#define MMC_R1_CARD_IS_LOCKED       (1 << 25)       /* sx, a */
#define MMC_R1_LOCK_UNLOCK_FAILED   (1 << 24)       /* erx, c */
#define MMC_R1_COM_CRC_ERROR        (1 << 23)       /* er, b */
#define MMC_R1_ILLEGAL_COMMAND      (1 << 22)       /* er, b */
#define MMC_R1_CARD_ECC_FAILED      (1 << 21)       /* ex, c */
#define MMC_R1_CC_ERROR             (1 << 20)       /* erx, c */
#define MMC_R1_ERROR                (1 << 19)       /* erx, c */
#define MMC_R1_UNDERRUN             (1 << 18)       /* ex, c */
#define MMC_R1_OVERRUN              (1 << 17)       /* ex, c */
#define MMC_R1_CID_CSD_OVERWRITE    (1 << 16)       /* erx, c, CID/CSD overwrite */
#define MMC_R1_WP_ERASE_SKIP        (1 << 15)       /* sx, c */
#define MMC_R1_CARD_ECC_DISABLED    (1 << 14)       /* sx, a */
#define MMC_R1_ERASE_RESET          (1 << 13)       /* sr, c */
#define MMC_R1_STATUS(x)            (x & 0xFFFFE000)
#define MMC_R1_CURRENT_STATE(x)     ((x & 0x00001E00) >> 9) /* sx, b (4 bits) */
#define MMC_R1_READY_FOR_DATA       (1 << 8)        /* sx, a */
#define MMC_R1_SWITCH_ERROR         (1 << 7)        /* sx, c */
#define MMC_R1_APP_CMD              (1 << 5)        /* sr, c */

#define MMC_R1_STATE_IDLE   0
#define MMC_R1_STATE_READY  1
#define MMC_R1_STATE_IDENT  2
#define MMC_R1_STATE_STBY   3
#define MMC_R1_STATE_TRAN   4
#define MMC_R1_STATE_DATA   5
#define MMC_R1_STATE_RCV    6
#define MMC_R1_STATE_PRG    7
#define MMC_R1_STATE_DIS    8
#define MMC_R1_STATE_SLP    9
...
typedef struct
{
    unsigned short initial_bad_blk_cnt;  /* initial bad block count */
    unsigned short later_bad_blk_cnt;  /* later bad block count */
    unsigned short spare_bad_blk_cnt; /* spare bad block count */
}bad_block_info;

typedef struct
{
    unsigned short phy_bad_blk_add;  /* bad block physical address */
    unsigned short failed_page_add_rec;  /* bad block failed address */
    unsigned char  failed_info; /* bad block failed info */
    unsigned char  failed_bus; /* failed bus */
    unsigned short reserved; /* reserved */
}bad_block_detailed_info;
...
}block_erase_count_info;


typedef struct
{
    unsigned short phy_blk_add[128];  /* max block erase count */
    unsigned short blk_erase_cnt[128]; /* avg block erase count */
//    unsigned char blk_erase_table_cnt;  /* min block erase count */
}block_erase_count_retrieve;
