	...
	if(state.chunk_offset >= sizeof(chunk_header_t)) {
		...
		case CHUNK_TYPE_RAW:
			...
			if(*section_size > 0) {
				...
				printf("sparse: writing segment: start=0x%08X, size=0x%08X\n", state.sector, copy_len/512);
		...
		case CHUNK_TYPE_DONT_CARE:
			if (state.last_header.total_sz != sizeof(chunk_header_t)) {
				...
				unsigned long skip = state.last_header.chunk_sz * (state.block_size / 512);
				/* if the first chunk is do not care, is is setting the offset,
				   (skipping some valid data). We do not erase in this case. */
				if (ERASE && state.first_access) {
					printf("sparse: erasing segment: start=0x%08X, size=0x%08X\n", state.sector, skip);
					ERASE(state.slot_no, state.sector, skip);
				} else {
					printf("sparse: skip 0x%08X sectors\n", skip);
				}
				state.sector += skip;
				state.first_access=1;
					...
					unsigned long sector, int len))
{
	...
	while((state.remaining_chunks > 0) && (section_size>0) && (ret == 0)) {
		ret = _unsparse_process_chunk(&source, &section_size, WRITE, ERASE);
	}
...
{
	if (_unsparse(source, section_size, mmc_write, mmc_erase))
		return 1;
#ifdef DEBUG
	if (_unsparse(source, section_size, mmc_compare, 0))
		return 1;
#endif
	return 0;
}
