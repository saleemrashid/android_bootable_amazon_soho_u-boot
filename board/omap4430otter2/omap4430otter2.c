...
#define MBID0_MASK (1<<MBID0_SHIFT)
#define MBID1_MASK (1<<MBID1_SHIFT)
#define MBID2_MASK (1<<MBID2_SHIFT)
#define MBID3_MASK (1<<MBID3_SHIFT)
#define PANELID0_MASK (1<<PANELID0_SHIFT)
#define PANELID1_MASK (1<<PANELID1_SHIFT)
#define TOUCHID0_MASK (1<<TOUCHID0_SHIFT)
#define TOUCHID1_MASK (1<<TOUCHID1_SHIFT)
#define MBID_GPIO6_MASK (MBID0_MASK | MBID1_MASK | MBID2_MASK | MBID3_MASK)
#define PANELID_GPIO6_MASK (PANELID0_MASK | PANELID1_MASK)
#define TOUCHID_GPIO2_MASK (TOUCHID0_MASK | TOUCHID1_MASK)
...
void mbid_init(void)
{
  ...
  unsigned val;
  val = __raw_readl(OMAP44XX_GPIO6_BASE + OMAP44XX_GPIO_OE) | MBID_GPIO6_MASK;
  __raw_writel(val, OMAP44XX_GPIO6_BASE + OMAP44XX_GPIO_OE);
}
...
unsigned get_mbid()
{
  ...
  val = __raw_readl(OMAP44XX_GPIO6_BASE + OMAP44XX_GPIO_DATAIN);
  mbid = ((val & MBID0_MASK)>>MBID0_SHIFT) |
         ((val & MBID1_MASK)>>(MBID1_SHIFT-1)) |
         ((val & MBID2_MASK)>>(MBID2_SHIFT-2)) |
         ((val & MBID3_MASK)>>(MBID3_SHIFT-3));
 ...
 *****************************************/
unsigned get_panelid()
{
  unsigned val;
  unsigned panelid;

  val = __raw_readl(OMAP44XX_GPIO6_BASE + OMAP44XX_GPIO_DATAIN);
  panelid = ((val & PANELID0_MASK)>>PANELID0_SHIFT) |
         ((val & PANELID1_MASK)>>(PANELID1_SHIFT-1));
 ...
 *****************************************/
unsigned get_touchid()
{
  unsigned val;
  unsigned touchid;

  val = __raw_readl(OMAP44XX_GPIO2_BASE + OMAP44XX_GPIO_DATAIN);
  touchid = ((val & TOUCHID0_MASK)>>TOUCHID0_SHIFT) |
         ((val & TOUCHID1_MASK)>>(TOUCHID1_SHIFT-1));
...
/*****************************************
 * Routine: get_otgid
 * Description: returns otg id pin value
 *****************************************/
unsigned get_otgid()
{
	return (__raw_readl(0x48055138) & 0x00100000);
}
/*****************************************
...
void set_SWBootingCfg()
{
  ...
  /* preserve mmc voltage across warm reset */
  ret = i2c_read(0x48, 0x9b, 1, &data, 1);
